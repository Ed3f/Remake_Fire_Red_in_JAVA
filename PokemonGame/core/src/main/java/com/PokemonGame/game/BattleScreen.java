...
import com.PokemonGame.game.actions.Action;
import com.PokemonGame.game.actions.ActionHandler;
import com.PokemonGame.game.actions.DelayAction;
import com.PokemonGame.game.actions.RunnableAction;
...
public class BattleScreen implements Screen, InputProcessor {
    ...
    public BattleScreen(PokemonGame game, Pokemon poke1, Pokemon poke2) {
        this.game = game;
        this.poke1 = poke1;
        this.poke2 = poke2;
        setupBattle();
    ...
    public void draw() {
        ...
        text.draw(game.batch, poke1.getHealth() + " / " + poke1.getMaxHealth(), 610,
            175);
        loadMoves(game.batch);

        if (usedAttack){
            game.batch.setColor(Color.WHITE);
            if(moveUsed.getName().equals("EMBER")){
                Animation<TextureRegion> animation= moveUsed.getAnimation();
                stateTime += Gdx.graphics.getDeltaTime();
                TextureRegion current_region= animation.getKeyFrame(stateTime);
                if (!animation.isAnimationFinished(stateTime)){
                    game.batch.draw(current_region, 200, 200, current_region.getRegionWidth(), current_region.getRegionHeight());
                }
                else {
                    stateTime= 0;
                    usedAttack= false;
            ...
            if(moveUsed.getName().equals("VINEWHIP")){
                Animation<TextureRegion> animation= moveUsed.getAnimation();
                stateTime += Gdx.graphics.getDeltaTime();
                TextureRegion current_region= animation.getKeyFrame(stateTime);
                if(!animation.isAnimationFinished(stateTime)){
            ...
            if(moveUsed.getName().equals("BUBBLE")){
                Animation<TextureRegion> animation= moveUsed.getAnimation();
                stateTime += Gdx.graphics.getDeltaTime();
                TextureRegion current_region= animation.getKeyFrame(stateTime);
                if(!animation.isAnimationFinished(stateTime)){
            ...
            if(moveUsed.getName().equals("GROWL")){
                Animation<TextureRegion> animation= moveUsed.getAnimation();
                stateTime += Gdx.graphics.getDeltaTime();
                TextureRegion current_region= animation.getKeyFrame(stateTime);
                if(!animation.isAnimationFinished(stateTime)){
            ...
            if(moveUsed.getName().equals("SCREECH")){
                Animation<TextureRegion> animation= moveUsed.getAnimation();
                stateTime += Gdx.graphics.getDeltaTime();
                TextureRegion current_region= animation.getKeyFrame(stateTime);
                if(!animation.isAnimationFinished(stateTime)){
        ...
        // metodo disegna animazione che prende come parametro i pokemon e la mossa usata
        game.batch.end();

    }

    public void loadMoves(Batch batch) {
        // access the size of the move array creates up to four buttons 2 re
        // always null
                ...
                RunnableAction action = new RunnableAction(new Runnable() {
                    ...
                    public void run() {
                        ...
                        if (poke1.getHealth() > 0) {
                            useMove(poke1, poke2, poke1.getMove(0), batch);
                        }
                    }
                });
                setUpTurn(action);
            ...
            public void onClicked() {
                ...
                RunnableAction action = new RunnableAction(new Runnable() {
                    ...
                    public void run() {
                        // starts actions
                        if (poke1.getHealth() > 0) {
                            useMove(poke1, poke2, poke1.getMove(1), batch);
                        }
                    }
                });
                setUpTurn(action);
            ...
            public void onClicked() {
                ...
                RunnableAction action = new RunnableAction(new Runnable() {
                    ...
                    public void run() {
                        // starts actions
                        if (poke1.getHealth() > 0) {
                            useMove(poke1, poke2, poke1.getMove(2), batch);
                        }
                    }
                });
                setUpTurn(action);
        ...
        button4 = new Button(text, moves[3].getName(), 300, 50) {
            ...
            public void onClicked() {
                RunnableAction action = new RunnableAction(new Runnable() {
                    ...
                    public void run() {
                        // starts actions
                        useMove(poke1, poke2, poke1.getMove(3), batch);
                    }
                });
                setUpTurn(action);
            }
    ...
    public void setUpTurn(RunnableAction playerAction) {
        ...
        // add computer action
        actionHandler.addAction(getComputerAction(game.batch));
     ...
     * @return
     */
    public Action getComputerAction(Batch batch) {
        ...
        return new RunnableAction(new Runnable() {
            ...
            public void run() {
                ...
                if (poke2.getHealth() > 0) {
                    useMove(poke2, poke1, move, batch);
                }
            }
        });
    }
     ...
     * @param batch
     */
    public void useMove(Pokemon attackingPokemon, final Pokemon defendingPokemon, Move move, Batch batch) {
